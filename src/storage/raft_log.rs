...
//!
use crate::{grpc::rpc_service::Entry, Result};
use sled::Subscriber;
...
pub trait RaftLog: Send + Sync + 'static {
    ...
    /// v20241105
    /// mostly used when we want to calculate the roughtly number of entry inside locallog after removing some entries. #size_of_db is not reliable.
    ///
    ...
    /// because obviously some entry might missing because of the pre-allocate index feature.
    ///
    ...
    fn prev_log_term(&self, follower_id: u32, prev_log_index: u64) -> u64;
    ...
    ///     follow it (§5.3)
    fn filter_out_conflicts_and_append(&self, _prev_log_index: u64, new_ones: Vec<Entry>) -> u64;

    fn insert_batch(&self, logs: Vec<Entry>) -> Result<()>;
    /// Clear out all the entries in local logs before last_applied
    /// this function will be invoked only when entries been applied to state machine successfully
