use crate::{
    grpc::rpc_service::{
        AppendEntriesRequest, ClientProposeRequest, ClientReadRequest,
        ClusteMembershipChangeRequest, MetadataRequest, VoteRequest,
    },
    MaybeCloneOneshot, RaftOneshot,
};
    ...
    pub fn mock_from_code(code: i32) -> Option<Self> {
        match code {
            3 => {
                ...
                Some(RaftEvent::ReceiveVoteRequest(
                    VoteRequest::default(),
                    dummy_tx,
                ))
            }
            ...
            8 => {
                let (dummy_tx, _) = MaybeCloneOneshot::new();
                Some(RaftEvent::ClientPropose(
                    ClientProposeRequest::default(),
                    dummy_tx,
                ))
            }
            9 => {
                let (dummy_tx, _) = MaybeCloneOneshot::new();
                Some(RaftEvent::ClientReadRequest(
                    ClientReadRequest::default(),
                    dummy_tx,
                ))
            }
