...
async fn test_join_cluster_scenario() -> Result<(), ClientApiError> {
    ...
    // Prepare raft logs
    let r1 = Arc::new(prepare_raft_log(&format!("{}/cs/1", JOIN_CLUSTER_CASE1_DB_ROOT_DIR), 0));
    let r2 = Arc::new(prepare_raft_log(&format!("{}/cs/2", JOIN_CLUSTER_CASE1_DB_ROOT_DIR), 0));
    let r3 = Arc::new(prepare_raft_log(&format!("{}/cs/3", JOIN_CLUSTER_CASE1_DB_ROOT_DIR), 0));
    let r4 = Arc::new(prepare_raft_log(&format!("{}/cs/4", JOIN_CLUSTER_CASE1_DB_ROOT_DIR), 0));
