...
async fn test_linearizable_read_served_without_quorum_in_minority_partition() {
    ...
    // Pure read, no log entries → prepare_batch_requests returns an empty PrepareResult.
    // Empty PrepareResult means no append_requests are generated, so Phase 4 in
    // execute_and_process_raft_rpc returns early: no AppendEntries is sent to any peer.
    // This simulates a partitioned cluster where followers are silent.
    let mut replication_handler = MockReplicationCore::new();
    replication_handler
        .expect_prepare_batch_requests()
        .times(1)
    ...
    // Precondition 1: noop committed at index 0 (bypasses the LeaderNotReady guard).
    // calculate_read_index() = max(commit_index=0, noop_index=0) = 0.
    // Default mock SM returns last_applied().index = 0.
    // Fast-path condition: last_applied(0) >= read_index(0) → TRUE.
    state.noop_log_id = Some(0);

    // Precondition 2: multi-voter cluster — quorum requires peer ACKs.
    // A single-voter leader is its own quorum and may safely serve immediately;
    // the bug only manifests when leadership confirmation requires a majority of peers.
    let mut membership = MockMembership::new();
    membership.expect_voters().returning(|| {
        vec![
            NodeMeta {
    ...
    state.init_cluster_metadata(&Arc::new(membership)).await.unwrap();
    assert!(!state.cluster_metadata.single_voter, "precondition: multi-voter cluster");

    let (role_tx, _role_rx) = mpsc::unbounded_channel();

    // Client sends a pure LinearizableRead (no concurrent write).
    let req = ClientReadRequest {
        client_id: 1,
        keys: vec![safe_kv_bytes(1)],
        consistency_policy: Some(ReadConsistencyPolicy::LinearizableRead as i32),
    ...
    // flush_cmd_buffers must return quickly — it must NOT block awaiting quorum.
    tokio::time::timeout(
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // CORE ASSERTION (Raft §8 requirement):
    //
    // The response channel must be EMPTY immediately after flush.
    // In a multi-voter cluster, a LinearizableRead must be deferred until an
    // AppendEntries heartbeat is sent and a quorum ACK confirms leadership.
    // Only after `handle_append_result` confirms quorum may the read be served.
    //
    // CURRENT BUG: try_recv() returns Ok(...) — the read was served in Phase 3
    // before any follower was contacted. This test FAILS with the current code.
    // ─────────────────────────────────────────────────────────────────────────
    assert!(
