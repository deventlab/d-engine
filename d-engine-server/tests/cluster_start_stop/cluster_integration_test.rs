...
/// Analytics Node Test: Add a read-only Learner that never auto-promotes
///
/// Scenario: Start a 3-node cluster, then add a 4th node as a Learner (Analytics node)
/// with Active status. Verify it receives data but remains as a Learner (non-voting).
/// This is useful for read-only analytics workloads.
#[tokio::test]
#[traced_test]
async fn test_analytics_node_learner_active() -> Result<(), ClientApiError> {
    const TEST_DIR: &str = "cluster_start_stop/analytics";
    const DB_ROOT_DIR: &str = "./db/cluster_start_stop/analytics";
    const LOG_DIR: &str = "./logs/cluster_start_stop/analytics";
    ...
    // Start initial 3-node cluster (nodes 1, 2, 3)
    let mut ctx = TestContext {
        graceful_txs: Vec::new(),
        node_handles: Vec::new(),
    };
    ...
    tokio::time::sleep(Duration::from_secs(WAIT_FOR_NODE_READY_IN_SEC)).await;
    println!("[test_analytics_node_learner_active] 3-node cluster started");

    // Verify initial cluster is ready and write some data
    ...
    println!("[test_analytics_node_learner_active] Data written to initial cluster");

    // TODO: Add node 4 as a Learner (Analytics node) with Active status
    // This requires:
    // 1. Create node config for node 4 as a Learner
    // 2. Start node 4
    // 3. Verify it syncs data from the cluster
    // 4. Verify it remains as a Learner (non-voting)
    // 5. Verify reads work on the analytics node
    //
    // Note: This requires JoinRequest support with explicit role=Learner
    // which may need additional API work in the framework

    println!(
        "[test_analytics_node_learner_active] Test completed (analytics node feature pending full implementation)"
    );
